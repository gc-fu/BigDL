...
def _pad_kv_cache_view(
    t: torch.Tensor, len: int, device: torch.device, pos: int = 2
) -> torch.Tensor:
...
class BigDLLlamaForCausalLM(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.config = config
        # If we are using selective batching, we might not be able to use BigDL-LLM model?
    ...
    def forward(
        ...
        # Recording the max_context_length for prefilling, used for later padding
        max_context_len = 0
        all_decoding = not seq_group_meta_data_lists[0].is_prompt
                ...
                if self.tmp_kv_cache[0][0].size(2) < max_seq_limit * 1.5:
                    ...
                    bigdl_kv_cache = [
                        [
                            tmp.narrow(
                                2,
                                self.tmp_kv_cache[0][0].size(2) - max_seq_limit,
                                max_seq_limit,
                            )
                            for tmp in tmp_list
                        ]
                        for tmp_list in self.tmp_kv_cache
                    ]
            else:
                bigdl_kv_cache = []
                for i in range(kv_cache_0):
                    ...
                    for j in range(kv_cache_1):
                        ...
                        if cur_view.size(2) > max_seq_limit:
                            cur_view = _pad_kv_cache_view(
                                cur_view, max_seq_limit, self.device
                            )
        ...
        for seq_id in cur_seq_ids:
            if kv_cache.get(seq_id) is None:
                kv_cache[seq_id] = [
                    [[] for _ in range(kv_cache_1)] for _ in range(kv_cache_0)
                ]
            for i in range(kv_cache_0):
                for j in range(kv_cache_1):
                    kv_cache[seq_id][i][j] = outputs.past_key_values[i][j][index]
            index = index + 1
