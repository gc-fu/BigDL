...
from bigdl.llm.vllm.model_executor.sampling_metadata import SamplingMetadata, SamplingTensors
...
def _apply_logits_processors(
    ...
    if found_logits_processors:
        assert logits_row_idx == logits.shape[0]  # noqa
    return logits
...
def _beam_search_sample(
    ...
    for seq_group, is_prompt in zip(selected_seq_groups, is_prompts):
        ...
        results.append((next_token_ids, parent_ids))
        sample_idx += num_parent_seqs
    assert sample_idx == logprobs.size(0)  # noqa
    return results
...
def _sample(
    ...
    for sampling_type in SamplingType:
        ...
        if sampling_type == SamplingType.GREEDY:
            ...
            raise ValueError(f"Unsupported sampling type: {sampling_type}")  # noqa
